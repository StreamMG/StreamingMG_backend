...
const thumbnailStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dir = path.resolve(process.cwd(), 'uploads/thumbnails');
    ensureDir(dir);
    cb(null, dir);
  },
...
const mediaStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    if (file.mimetype.startsWith('video/')) {
      dir = path.resolve(process.cwd(), 'uploads/private');
    } else {
      dir = path.resolve(process.cwd(), 'uploads/audio');
    }
    ensureDir(dir);
    cb(null, dir);
  },
  ...
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      ...
      if (file.fieldname === 'thumbnail') {
        dir = path.resolve(process.cwd(), 'uploads/thumbnails');
