...
async function run() {
  ...
  assert(
    "Serveur accessible (GET /api/health → 200)",
    health.status === 200,
  ...
  assert(
    "Login user@test.com",
    !!tokenUser,
    `token=${tokenUser ? tokenUser.substring(0, 20) + "…" : "null"}`,
  );
  assert(
    "Login premium@test.com",
    !!tokenPremium,
    `token=${tokenPremium ? tokenPremium.substring(0, 20) + "…" : "null"}`,
  );
  assert(
    "Login admin@test.com",
    !!tokenAdmin,
    `token=${tokenAdmin ? tokenAdmin.substring(0, 20) + "…" : "null"}`,
  );
  ...
  }
  {
    const r = await req(
      "POST",
      `/api/download/000000000000000000000000`,
      null,
      {
    ...
    assert(
      "POST /download ID inexistant → 404",
      r.status === 404,
      `status=${r.status}`,
    );
  }
  ...
  if (freeId && tokenUser) {
    ...
    assert(
      "POST /download (free+auth) → 200 ou 404 FILE_NOT_FOUND",
      ok200 || ok404,
    ...
    if (ok200) {
      assert(
        "aesKeyHex présent",
        !!r.body?.aesKeyHex,
        `val=${r.body?.aesKeyHex?.substring(0, 8)}…`,
      );
      assert(
        "aesKeyHex = 64 hex char",
        r.body?.aesKeyHex?.length === 64,
        `len=${r.body?.aesKeyHex?.length}`,
      );
      assert(
        "ivHex présent",
        !!r.body?.ivHex,
        `val=${r.body?.ivHex?.substring(0, 8)}…`,
      );
      assert(
        "ivHex = 32 hex char",
        r.body?.ivHex?.length === 32,
        `len=${r.body?.ivHex?.length}`,
      );
      assert(
        "signedUrl présent",
        !!r.body?.signedUrl,
        `url=${r.body?.signedUrl?.substring(0, 60)}…`,
      );
      assert(
        "signedUrl contient /private/",
        r.body?.signedUrl?.includes("/private/"),
        `url=${r.body?.signedUrl}`,
      );
      assert(
        "signedUrl contient expires=",
        r.body?.signedUrl?.includes("expires="),
        `url=${r.body?.signedUrl}`,
      );
      assert(
        "signedUrl contient sig=",
        r.body?.signedUrl?.includes("sig="),
        `url=${r.body?.signedUrl}`,
      );
      assert(
        "expiresIn = 900",
        r.body?.expiresIn === 900,
        `val=${r.body?.expiresIn}`,
      );
      assert(
        "contentType présent",
        !!r.body?.contentType,
        `val=${r.body?.contentType}`,
      );

      // Valider le format hex (aesKeyHex ne doit contenir que [0-9a-f])
      const hexOk = /^[0-9a-f]{64}$/.test(r.body.aesKeyHex || "");
      assert(
        "aesKeyHex = hex pur [0-9a-f]",
        hexOk,
        `val=${r.body?.aesKeyHex?.substring(0, 16)}…`,
      );
      ...
      console.log(
        "  ℹ️  Fichier source absent (transcodage en cours ou contenu sans fichier physique).",
      );
    }
  }
  ...
  if (premiumId && tokenUser) {
    ...
    assert(
      "Premium + user non-abonné → 403 subscription_required",
  ...
  if (paidId && tokenPremium) {
    ...
    assert(
      "Paid + premium sans achat → 403 purchase_required (RÈGLE-05)",
  ...
  if (signedUrl) {
    ...
    assert(
      "GET /private URL valide → 200 ou 206",
    ...
    assert(
      "GET /private sans params → 403 MISSING_PARAMS",
      r2.status === 403 && r2.body?.code === "MISSING_PARAMS",
      `status=${r2.status} code=${r2.body?.code}`,
    );

    // 6c. URL expirée
    const rExp = await req(
      "GET",
      `${parsedUrl.origin}${parsedUrl.pathname}?expires=1000&sig=aabbcc`,
    );
    assert(
      "GET /private URL expirée → 403 URL_EXPIRED",
    ...
    const rFake = await req(
      "GET",
      `${parsedUrl.pathname}?expires=${exp}&sig=${"ab".repeat(32)}`,
    );
    assert(
      "GET /private signature falsifiée → 403 INVALID_SIGNATURE",
    ...
    assert(
      "GET /private Range 0-1023 → 206 Partial Content",
      rRange.status === 206 || rRange.status === 200,
      `status=${rRange.status} CR=${rRange.headers["content-range"] || "N/A"}`,
    );

    // 6f. Range invalide → 416
    const rBadRange = await req("GET", signedUrl, null, {
      Range: "bytes=999999999-9999999999",
    });
    assert(
      "GET /private Range invalide → 416",
    ...
    assert(
      "Réponse inclut Accept-Ranges: bytes",
    ...
    assert(
      "Réponse inclut Cache-Control: no-store",
      r.headers["cache-control"]?.includes("no-store"),
      `cache-control=${r.headers["cache-control"]}`,
    );
  } else {
    console.log(
      "  ⚠️  signedUrl non disponible (fichier absent du disque) — tests /private ignorés.",
    );
  }
  ...
  const ctrlSrc = fs.readFileSync(
    path.join(__dirname, "../src/controllers/downloadController.js"),
    "utf8",
  );
  assert(
    "downloadController ne fait pas de Content.save() / findByIdAndUpdate avec clé",
    !/(aesKeyHex|ivHex).*save\(|findByIdAndUpdate.*aesKey/i.test(ctrlSrc),
    "Analyse statique du contrôleur",
  );
  assert(
    "downloadController ne fait pas de console.log(aesKey)",
  ...
  console.log("\n╔══════════════════════════════════════════════════════╗");
  console.log(
    `║  BILAN : ${String(PASS).padEnd(3)} ✅  /  ${String(FAIL).padEnd(3)} ❌  /  ${PASS + FAIL} tests total         ║`,
  );
  console.log("╚══════════════════════════════════════════════════════╝");
